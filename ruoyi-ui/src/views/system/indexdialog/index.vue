...
<el-col v-for="index in  sys_come_tableList" :key="index.id" >
  <!-- <a-button type="primary" >{{index.comeName}}</a-button> -->
<el-tag type="success">{{index.id}}</el-tag>

<el-tag type="info">欢迎进入厂区，您的到访时间：{{index.comeTime}}</el-tag><br>
<el-tag type="info">计时开始：</el-tag>
...
<script>
import { listSys_come_table, getSys_come_table, delSys_come_table, addSys_come_table, updateSys_come_table, exportSys_come_table } from "@/api/system/sys_come_table";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除出入登记编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delSys_come_table(ids);
        }).then(() => {
          this.getList();
